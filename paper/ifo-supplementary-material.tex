...
%notch_and_wiener_superplot_v2.pdf}
    ...
    The eight row shows the results of applying the logMMSE estimator (identical to the fourth row in Fig.~7 in the main article). 
	}
\end{figure}
...
An inituitive way to remove noise is by directly subtracting the background noise from the recorded spectrum. 
...
In this work we opt for a IIR notch filter. 
...
We see that the mains harmonics are significantly attenuated in comparison background subtracted results shown in the fourth row in Fig.~\ref{fig:op_mic_results}.
