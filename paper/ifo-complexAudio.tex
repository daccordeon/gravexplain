...
\end{equation}
Assuming that $\textbf{x}$ is wide-sense stationary \han{[perhaps a simple definition of wide-sense stationary should be included in brackets here? to do; ask Changrong]} and that $\textbf{w}$ is stationary and uncorrelated with the signal, we represent the $n$th order Wiener filter by its coefficients $\textbf{h}=\{h(0),\dots(n)\}$. 
