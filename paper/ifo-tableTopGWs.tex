...
Detection and analysis techniques used by the gravitational-wave community can be demonstrated using table-top equipment (see also the further reading section in supplementary material for a selection of other table-top Michelson interferometer designs used for science communication).
...
The output of a Class 2 $532\,{\rm nm}$ laser (top-left) is reflected from a mirror at a right angle. The beam then passes through a converging lens (focal length: $125.0\,{\rm mm}$) before reaching the beamsplitter which is approximately $50$/$50$ in reflection and transmission. 
...
The intensity timeseries is recorded by webcam (Sections~\ref{sec:single_tone} and~\ref{sec:viterbi_wandering}) or photodiode, which offers a higher sampling rate suitable for capturing more complex audio (Section~\ref{sec:optical_microphone}).
...
This image was taken with the webcam aligned off the beam axis. %, which may explain why the (predicted circular) pattern appears elliptical.
...
%In the following section, we start simply by injecting and recovering an audio signal of constant frequency (a single tone).
