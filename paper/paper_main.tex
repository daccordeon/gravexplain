...
As mentioned in Section~\ref{sec:introduction}, continuous gravitational wave searches look for near monochromatic signals. The audio analogue of this is a simple sinusoidal tone; a constant note.

We measured the intensity at a point on the screen with a commercial USB webcam placed as to view the screen as in Figure~\ref{fig:ifo_schematic_webcam}. This webcam samples at 30Hz, which limits the spectral content of signals we can observe to below 15Hz, the Nyquist frequency.
% See later Section~\ref{sec:photodiode} where we raise this limit into audible frequencies (20 Hz to 20kHz) with a photodiode.
While playing a tone under 15Hz through the speaker for a minute, we recorded the interference pattern with the webcam. Then, at an off-centre pixel (shown as a red dot in Figure~\ref{fig:interference_pattern}), we took the green-channel of the video as approximate to the total intensity, as the laser is monochromatic green light (at 532nm).
