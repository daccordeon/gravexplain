...
\han{to do add notes from Andrew on this}
...
The noise $n(t)$ is a fluctuating zero-mean time series with the following property: if we define an inner product between two time series $u(t)$ and $v(t)$ as 
\begin{equation}
\ip{u}{v} = \frac{1}{T} \int_0^T \mathrm{d} t \, u(t) v(t)\,,
\label{eqn:ipuv}
\end{equation}
where $T$ is the total time of the observation, then the likelihood $\mathcal{L}$ of measuring the noise-related value $\ip{n}{n}$ is given by 
\begin{equation}
\mathcal{L} = \exp\left( -\frac{1}{2}\ip{n}{n}\right)\,.
\label{eqn:ipnn}
\end{equation}
Equations~\ref{eqn:ipuv} and~\ref{eqn:ipnn} define Gaussian noise through the fundamental measurement of $\ip{n}{n}$.
...
% We replace $n$ by $x-s$ (Eqn.~\ref{eqn:GNdata}), so that 
...
(i) Fundamentally the goal is to maximize $\mathcal{L}$ in Eqn.~\ref{eqn:likeOne} by varying the signal.
For the special case of the Gaussian likelihood (Eqn.~\ref{eqn:ipnn}), which peaks at $\ip{n}{n}= 0$, this is equivalent to minimizing the distance between $x(t)$ and $s(t)$ defined through Eqn.~\ref{eqn:ipuv}.
In general, however, it is not equivalent to minimizing the distance between $x(t)$ and $s(t)$, for example if $\mathcal{L}$ peaks at $\ip{n}{n} \neq 0$, or if $\mathcal{L}$ has multiple maxima.
(ii) The maximum likelihood $\mathcal{L}_{\rm max}$ in Eqn.~\ref{eqn:maxLogLike} defines the detection statistic. 
When its value exceeds a threshold (chosen freely by the analyst) at some value of $\omega$, a signal is deemed to have been deteccted at that value of $\omega$.
...
%baring the constant factor $\ip{x}{x}$.  
\end{comment}
...
% moved to supp. matt. The interested reader is referred to the excellent review paper by Rabiner~\cite{Rabiner:1989} and textbook by Quinn and Hannam~\cite{QuinnEtAl:2001} to learn more. 
...
\begin{comment} % move to supp matt due to large number of references needed here. 
