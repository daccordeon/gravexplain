...
Before we begin analysing the data, we have no prior knowledge as to which frequency bin the signal starts from (i.e. the prior is flat between the minimum and maximum frequency bins).
...
%We note that there is no marginalization the left-hand-side and right-hand-side of Eqn.~\ref{eqn:pathprobability} are both evaluated for a specific path and then we maximize over all such paths to find the optimal Viterbi path. 
