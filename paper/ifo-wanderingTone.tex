...
% the signal and the analogy we use
Continuous gravitational waves may not be monochromatic, unlike the signal considered in Section~\ref{sec:single_tone}.  
...
One method to search for a wandering signal is to split the time series data into several shorter segments which are analysed individually.%, and then recombine them under some continuity assumption to coherently analyse the whole set. 
...
Which detection statistic used depends on the type of target~\cite{JKS:1998,SuvorovaEtAl:2017}.
...
The detection statistic is normalised for convenience. 
The detection statistics from each segment are combined to form a grid in time and frequency (a spectrogram). 
...
% why add to the confusion? why not make things consistent? 
%From here on, we will call these time “steps” instead of time bins, but they should not be confused with the timesteps of the original time series. Reading through it seems we were using both steps and bins in this section anyway. 
We label the elements of the time and frequency  grid as $t_i$ and $f_j$ respectively, where $i=0,1,2,...N_t$ and $j=0,1,2,...,N_f$. 
% state is means frequency not grid point doesn't it? 
%At each state (grid point) the detection statistic (the normalised Fourier amplitude) is notated $F(t_i,f_j)$.
...
% I think we should go for present tense throughout - hannah

A sinusoidal signal with slowly decaying amplitude is played through the speaker to mimic a slowly wandering signal. 
...
The overlaid pink dots show the injected signal and the white crosses show the recovered Viterbi path.% I think this is already defined and seems like repetition:(the most likely path through the data according to the Viterbi algorithm). 
