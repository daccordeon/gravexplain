...
\han{arrow?}At the first stage of the analysis, we define the probability of the system having frequency $f_j$ at the initial time $t_0$ to be equal to the (normalized) detection statistic of that state (i.e.\  ${\rm Pr}[f(t_0) = f_j] = F[t_0,f_j]$).
...
The Viterbi algorithm's strength lies in its ability to track such signals through the data \han{even in the presence of comparatively loud noise fluctuations in the time-frequency bins}.
