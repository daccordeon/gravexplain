...
\item We have lightened the colour scheme used in the Fig. 4 schematic to aid with black and white printing. It is also now a similar colour scheme to the spectrogram in Fig. 5 in order to make it clearer that we are simulating a spectrogram with this schematic. When we introduce Fig. 5 we now also refer back to Fig. 4 to indicate that the schematic represents a similified version of the results shown in Fig. 5. 
\item \han{[Hannah] The hidden Markov model and Viterbi algorithm's strength is in it's ability to find randomly wandering signals. The single constant tone of section III is recovered easily using a Fourier transform, however if the signal frequency changes randomly over time, this will reduce the Fourier transform's ability to identify the signal as the signal power iss spread across multiple frequency bins. We have added some text at the start of Section IV (paragraph 2) and at the end of Section IV. B. to explain that the Fourier transform of the whole dataset is not well suited to this type of signal.
\item We have also revied the entire Section IV to improve clarity in our Viterbi algorithm explanation and motivation.
\end{itemize}
...
\item \todo{They will also expect a change to the paper as it is a point of confusing - e.g. clarify the reason in the figure caption.}
\end{itemize}
...
\begin{itemize}
\item We have move the majority of Section VC to Supplementary Material and now only include highlight results in the main text (the combined notch and Wiener filter and the logMMSE estimator). In the Supplementary Material we have expanded the description of the impulse response of a filter mentioning both IIR and FIR. We have also added some text saying that Eq. 2 in the Supplementary Material (previously Eq. 4 in the main text) is a Z-tranform. We have corrected the inconsistency in Eq. 1 in the Supplementary Material (previously Eq. 3 in the main text). 
