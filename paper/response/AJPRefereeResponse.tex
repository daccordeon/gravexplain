...
\item Figure 5 schematic (previously Figure 4): We have lightened the color scheme used in the Figure 5 schematic to aid with black and white printing. The colors represent a gradient rather than a discrete shading and we now use a blue/green color palette to clarify this. We intend that the Figure 5 schematic be interpreted as a simplified version of the Figure 6 (previously Figure 5) spectrogram result. To clarify this we have made the color scheme of the schematic more consistent with that of Figure 6 and we have referred back to the schematic when we first introduce Figure 6. 
\item The grey paths in Figure 5 are those that have been rejected at each timestep. We have clarified this in the text and caption. 
...
\begin{itemize}
\item Figure 6 (previously Figure 5) colors: The color coding represents the logarithm of the absolute value of the discrete Fourier transform. We have added this to the caption.
\item Comparison without VA: Without using the Viterbi algorithm the signal becomes spread out over the frequency bins and might not rise above the noise. We have now explained this and added a new figure (Figure 4) that shows the Fourier spectrum of the entire timeseries to illustrate that the Fourier transform is not well suited here. (See also our answer to point 3 above).
...
\begin{itemize}
\item The Butterworth filter has strongly attenuated the signal outside the passband, this sharp drop-off at low frequencies would appear shallower when shown against a logarithmic scale for frequency as in Figure~3 in the Supplementary Material. We have made a note of this in the Supplementary Material where this plot has been moved to.
\item[]
\end{itemize}
...
\item All of the code is readily available on the provided GitHub. The provided code is documented and commented to ease reproducing the results. For the double-anonymous peer review, we removed the link to our git repository from the manuscript. 
\item[]
\end{itemize}
...
\begin{itemize}
\item We have clarified the explanation of the HMM in the text. The detection statistic $F(t_i, f_j)$ captures the likelihood of a signal being present in each time-frequency bin conditional on the observed data, where we have dropped the conditionally from the notation to avoid clutter. In Section IV A we have expanded our description of gravitational wave analysis and referred to both the transmission matrix and emission probability by name to reduce confusion.
\item[]
\end{itemize}
...
\begin{itemize}
\item Agreed, each value is divided by the maximum value in the grid such that $\max_{i,j} F(t_i, f_j) = 1$. We have added this information to section IV B.
\item[]
\end{itemize}
